...
// ===== 场景十：主子表联动编辑（Master-Detail Dialog）=====
export const masterDetailDialogExample = `<template>
  <div>
    <el-button type="primary" @click="openMasterDetailDialog">打开主子表弹窗</el-button>
    <div v-if="lastOrder" class="result-info">
      最近订单：{{ lastOrder.supplier }} — ¥{{ lastOrder.total }}
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    openMasterDetailDialog() {
      // 弹窗内部数据（使用 Vue.observable 保持响应式）
      const formData = Vue.observable({
        supplier: '',
        orderDate: '',
        remark: ''
            ...
            <div style="padding: 10px;">
              ...
              <el-divider content-position="left">
                <span style="font-size: 13px; color: #606266;">
                  订单明细（共 {detailList.length} 行，合计 ¥{total}）
                </span>
              </el-divider>
